...
const EditPurchaseOrder = ({ props, purchaseOrder }) => {
  ...
  const { loading: load_proveedores, data: data_proveedores } = useQuery(OBTENER_PROVEEDORES_2, {
    pollInterval: 1000
  });
  const { loading: load_productos, data: data_productos } = useQuery(
    OBTENER_TODAS_MATERIAS_PRIMAS,
    { pollInterval: 1000 }
  );
  const { loading: load_impuestos, data: data_impuestos } = useQuery(OBTENER_IMPUESTOS, {
    pollInterval: 1000
  });
  ...
  const [fechaPedido, setFechaPedido] = useState(
    `${new Date().getFullYear()}-${new Date().getMonth() + 1 > 10 ? new Date().getMonth() + 1 : '0' + (new Date().getMonth() + 1)}-${new Date().getDate() > 10 ? new Date().getDate() : '0' + new Date().getDate()}`
  );
  ...
  const estadoPedidos = ['Borrador', 'Confirmado', 'Cancelado', 'Recibido'];
  ...
  useEffect(() => {
    ...
    setNumeroComprobante(
      purchaseOrder.numeroComprobante && purchaseOrder.numeroComprobante.trim().length > 0
        ? purchaseOrder.numeroComprobante
        : ''
    );
    ...
    if (purchaseOrder.lineasPedido && purchaseOrder.lineasPedido.length > 0) {
      setLineasPedido(
        purchaseOrder.lineasPedido.map((linea) => {
          return {
            id: linea.id,
  ...
  const guardarLinea = () => {
    if (modoLinea === 'Agregar') {
      ...
      if (existe) {
        showInfoAlert({
          title: 'Oops',
          text: 'Ya existe una línea con ese producto',
          icon: 'warning',
          timer: 3000,
          position: 'center'
        });
  ...
  const editarLinea = (linea, index) => {
    ...
    if (!linea.impuesto || linea.impuesto === null) {
      ...
      setImpuesto({
        value: linea.impuesto.id,
        label: `${linea.impuesto.nombre} (${linea.impuesto.valor}%)`
      });
    }
  ...
  const onSaveOrdenCompra = async () => {
    try {
      ...
      const input = {
        ...
        fechaPedido:
          fechaPedido !== null && fechaPedido.trim().length > 0 ? fechaPedido + ' 00:00:00' : null,
      ...
      const { data } = await actualizar({
        variables: { id: purchaseOrder.id, input, inputLineasEditar },
        errorPolicy: 'all'
      });
  ...
  const confirmarPedido = async () => {
    if (!lineasPedido || lineasPedido.length === 0) {
      showInfoAlert({
        title: 'Oops',
        text: 'No se puede confirmar un pedido sin líneas',
        icon: 'error',
        timer: 3000,
        position: 'center'
      });
  ...
  const actualizarEstado = async (estadoPedido) => {
    try {
      const { data } = await actualizarEstadoCompra({
        variables: { id: purchaseOrder.id, estado: estadoPedido },
        errorPolicy: 'all'
      });
      ...
      showInfoAlert({
        title: 'Oops',
        text: 'Hubo un error inesperado al actualizar estado orden de compra',
        icon: 'error',
        timer: 3000,
        position: 'center'
      });
        ...
        <div className="page-content">
          <Container fluid={true}>
            <Breadcrumbs
              ...
              breadcrumbItem="Órdenes de compra"
              breadcrumbItemUrl="/purchaseorders"
            />
            <Row>
        ...
        <Container fluid={true}>
            ...
            <div className="col d-flex justify-content-end mb-3">
              {estadoPedidos.map((estado, i) => (
                <div
                  ...
                  className={
                    estado === estadoPedido
                      ? `states_cards active ${i == 0 ? 'first' : ''} ${i == estadoPedidos.length - 1 ? 'last' : ''}`
                      : `states_cards ${i == 0 ? 'first' : ''} ${i == estadoPedidos.length - 1 ? 'last' : ''}`
                  }
                >
                  <span>{estado}</span>
                </div>
            ...
            <div className="col mb-3 text-start">
              {estadoPedido === 'Borrador' && (
                <>
                  ...
                  <button
                    type="button"
                    className="btn btn-outline-info waves-effect waves-light me-2"
                    onClick={() => {
                      cancelarPedido();
                    }}
                  >
                  ...
                  <button
                    ...
                    onClick={() => {
                      onDelete(purchaseOrder.id, purchaseOrder.numeroComprobante);
                    }}
                  >
                    Eliminar
                  </button>
          ...
          <Row>
            <div className="col-md-4 col-sm-12 mb-3">
              <label htmlFor="purchaseOrderDate" className="form-label">
                Fecha de pedido
              </label>
              <input
          ...
          <Row className="align-items-center">
            <div className="col mt-5 mb-3">
              <h6>
                <strong>Líneas de pedido</strong>
              </h6>
            </div>
            {estadoPedido === 'Borrador' && (
              <div className="col-2 mt-5 mb-3 text-end">
                ...
                >
                  Limpiar <i className="ri-delete-bin-line align-middle"></i>
                </button>
              </div>
            )}
            <hr />
          ...
          {estadoPedido === 'Borrador' && (
            <Row className="align-items-end">
              ...
              <div className="col mb-3">
                <label htmlFor="cantidadLinea">Cantidad</label>
                <input
              ...
              <div className="col mb-3">
                <label htmlFor="precioUnitarioLinea">Precio unitario</label>
                <input
                  disabled={disabledLine()}
                  id="precioUnitarioLinea"
              ...
              <div className="col mb-3">
                <label htmlFor="porcentajeDescuentoLinea">Descuento (%)</label>
                <input
                  disabled={disabledLine()}
                  id="porcentajeDescuentoLinea"
              ...
              <div className="col mb-3">
                ...
                <Select
                  menuPosition="fixed"
                  isDisabled={disabledLine()}
                  id="impuesto"
                  placeholder="Impuesto"
                  value={impuesto}
                  options={getImpuestos()}
                  isSearchable={true}
                  isClearable={true}
                  onChange={(e) => setImpuesto(e)}
                />
              </div>
              <div className="col mb-3">
                {modoLinea === 'Agregar' ? (
                  <button
                    ...
                    type="button"
                    className="btn btn-outline-secondary waves-effect waves-light"
                    disabled={disabledSaveLine()}
                    onClick={() => guardarLinea()}
                  >
                    Agregar <i className="ri-add-line align-middle"></i>
                  </button>
                ) : (
                  <button
                    style={{ width: '100%' }}
                    ...
                    onClick={() => guardarLinea()}
                  >
                    Guardar <i className="ri-save-line align-middle"></i>
                  </button>
                )}
              </div>
            </Row>
