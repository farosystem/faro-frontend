...
const NewStockMoveOut = () => {
  ...
  const [
    getWarehouseLines,
    { loading: load_lineas_almacenes, error: error_lineas_almacenes, data: data_lineas_almacenes }
  ...
  useEffect(() => {
    setCantidadDisponibleAlmacen(
      data_lineas_almacenes
        ? data_lineas_almacenes.obtenerLineasAlmacen.find(
            (value, index) => value.producto.id === productId
          )?.cantidad || '0'
        : ''
    );
  }, [data_lineas_almacenes]);
  ...
  const onSave = async () => {
    ...
    try {
      ...
      const { data } = await insertar({
        variables: { input, almacen: almacen.value.id },
        errorPolicy: 'all'
      });
      ...
      showInfoAlert({
        title: 'Oops',
        text: 'Ocurrió un error inesperado al registrar la salida',
        icon: 'error',
        timer: 3000
      });
      setDisableSave(false);
      ...
      <React.Fragment>
        <div className="page-content">
          <Container fluid={true}>
            <Breadcrumbs
              title={`Agregar salida - ${productName}`}
              breadcrumbItem={`Movimientos - ${productName}`}
              breadcrumbItemUrl={`/product/movements/${stockType}/${productName}/${productId}`}
            />
            <Row>
                ...
                {cantidadDisponibleAlmacen && (
                  <div className="col-md-6 col-sm-12 mb-3">
                    ...
                    <input
                      readOnly={true}
                      id="cantidad"
                      value={cantidadDisponibleAlmacen}
                      placeholder="Cantidad disponible en almacén"
