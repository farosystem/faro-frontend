...
const NewStockMoveIn = () => {
  ...
  const { loading: load_clientes, data: data_clientes } = useQuery(OBTENER_CLIENTES, {
    pollInterval: 1000
  });
  const { loading: load_almacenes, data: data_almacenes } = useQuery(OBTENER_ALMACENES, {
    pollInterval: 1000
  });
  const { loading: load_producto, data: data_producto } = useQuery(OBTENER_MATERIA_PRIMA, {
  ...
  useEffect(() => {
    setDisableSave(
      !almacen ||
        ...
        cantidad === null ||
        cantidad <= 0 ||
        (!cedido && (!proveedor || proveedor === null)) ||
        (cedido && (!cliente || cliente === null))
    );
  ...
  const onSave = async () => {
    ...
    if (!pasar) {
      try {
        ...
        showInfoAlert({
          title: 'Oops',
          text: 'Ocurrió un error inesperado al registrar la entrada',
          icon: 'error',
          timer: 3000
        });
        setDisableSave(false);
      ...
      showInfoAlert({
        title: 'Oops',
        text: 'Los valores numericos deben ser mayor a cero',
        icon: 'error',
        timer: 3000,
        position: 'center'
      });
      ...
      <Fragment>
        <div className="page-content">
          <Container fluid={true}>
            <Breadcrumbs
              title={`Agregar entrada - ${productName}`}
                ...
                <div className="col mt-2">
                  <div className="form-check">
                    <input
                      ...
                      id="cedidoCheck"
                    />
                    <label className="form-check-label" htmlFor="cedidoCheck">
