...
import {
  GUARDAR_REGISTRO_CONTABLE,
  ACTUALIZAR_REGISTRO_CONTABLE
...
type ContactType = 'usuario' | 'cliente' | 'proveedor';
type AccountType = 'PAGAR' | 'COBRAR';
type PaymentType = 'EFECTIVO' | 'CHEQUE' | 'TRANSFERENCIA' | 'SINPE' | 'TARJETA';

interface EditAccountingControlProps {
  accountingControl: any;
  tipo: string;
}

const EditAccountingControl = ({ accountingControl, tipo }: EditAccountingControlProps) => {
  React.useEffect(() => {
  ...
  const [contactsType, setContactsType] = useState<SelectOption<ContactType> | null>(null);
  const [accontType, setAccountsType] = useState<SelectOption<AccountType> | null>(null);
  const {
    loading: load_proveedores,
    data: data_proveedores,
    startPolling: startPolling_proveedores,
    stopPolling: stopPolling_proveedores
  } = useQuery(OBTENER_PROVEEDORES, { pollInterval: 1000 });
  const {
    loading: load_clientes,
    data: data_clientes,
    startPolling: startPolling_clientes,
    stopPolling: stopPolling_clientes
  } = useQuery(OBTENER_CLIENTES, { pollInterval: 1000 });
  const {
    loading: load_usuarios,
    data: data_usuarios,
    startPolling: startPolling_usuarios,
    stopPolling: stopPolling_usuarios
  } = useQuery(OBTENER_USUARIOS_ACTIVOS, { pollInterval: 1000 });
  ...
  const [
    getPurchaseOrder,
    { loading: loading_orden_compra, error: error_orden_compra, data: data_orden_compra }
  ] = useLazyQuery(OBTENER_ORDEN_COMPRA);

  useEffect(() => {
    setAccountsType({
      label: accountingControl.tipoRegistroContable,
      value: accountingControl.tipoRegistroContable
    });

    setFechaRegistro(getFecha(accountingControl.fechaRegistro));
    setFechaPago(getFecha(accountingControl.fechaPago));
    setTipoPago(
      accountingControl.tipoPago
        ? { label: accountingControl.tipoPago, value: accountingControl.tipoPago as PaymentType }
        : null
    );
    setEstado(accountingControl.estado);
    setReferenciaID(accountingControl.referenciaID ? accountingControl.referenciaID : '');
    setReferenciaNombre(
      accountingControl.referenciaNombre ? accountingControl.referenciaNombre : ''
    );
    setReferenciaModelo(
      accountingControl.referenciaModelo ? accountingControl.referenciaModelo : ''
    );
    ...
    if (accountingControl.proveedor !== null) {
      ...
      setProveedor({
        label: accountingControl.proveedor.empresa,
        value: accountingControl.proveedor
      });
    }
    ...
    if (
      accountingControl.estadoRegistroContable !== 'BORRADOR' &&
      accountingControl.referenciaID &&
      accountingControl.referenciaModelo === 'ordenCompra'
    ) {
  ...
  useEffect(() => {
    ...
    startPolling_proveedores,
    stopPolling_proveedores,
    startPolling_clientes,
    stopPolling_clientes,
    startPolling_usuarios,
    stopPolling_usuarios
  ]);

  const getFecha = (fechaString: string | null | undefined) => {
    if (typeof fechaString !== 'string' || fechaString.trim().length === 0) return '';
  ...
  const contactsTypes: SelectOption<ContactType>[] = [
    { label: 'Usuario', value: 'usuario' },
    { label: 'Cliente', value: 'cliente' },
    { label: 'Proveedor', value: 'proveedor' }
  ];

  const accontsTypes: SelectOption<AccountType>[] = [
    { label: 'Por pagar', value: 'PAGAR' },
    { label: 'Por cobrar', value: 'COBRAR' }
  ];

  const statesAccountingControl = ['BORRADOR', 'PENDIENTE', 'PAGADO'];

  const tiposDePago: SelectOption<PaymentType>[] = [
  ...
  const onSaveRegistroContable = async (estadoRC: string) => {
    try {
      setDisableGuardarOrden(true);
      const input = {
        estado: 'ACTIVO',
        estadoRegistroContable: estadoRC,
        fechaRegistro: fechaRegistro ? `${fechaRegistro} 00:00:00` : null,
        fechaPago: fechaPago ? `${fechaPago} 00:00:00` : null,
        tipoPago: tipoPago?.value ?? null,
        tipoRegistroContable: accontType?.value ?? null,
        referenciaID: referenciaID,
        referenciaNombre: referenciaNombre,
        referenciaModelo: referenciaModelo,
        cliente: cliente?.value?.id ?? null,
        proveedor: proveedor?.value?.id ?? null,
        usuario: usuario?.value?.id ?? null,
        monto: monto || 0,
      ...
      const { data } = await actualizar({
        variables: { id: accountingControl.id, input },
        errorPolicy: 'all'
      });
      ...
      showInfoAlert({
        title: 'Oops',
        text: 'Ocurrió un error inesperado al actualizar el registro contable',
        icon: 'error',
        timer: 3000,
        position: 'top-end'
      });
  ...
  useEffect(() => {
    if (estadoRegistroContable === 'BORRADOR') {
      if (
        fechaRegistro === null ||
        fechaRegistro.trim().length === 0 ||
        monto === null ||
        monto === 0 ||
        (usuario === null && proveedor === null && cliente === null)
      ) {
        setDisableGuardarOrden(true);
        return;
      }
    ...
    if (
      ...
      (tipoPago.value !== 'EFECTIVO' &&
        (comprobantePago === null || comprobantePago.trim().length === 0))
    ) {
      setDisableGuardarOrden(true);
      return;
    }
  ...
  useEffect(() => {
    setDisablePagarOrden(
      fechaRegistro === null ||
        fechaRegistro.trim().length === 0 ||
        fechaPago === null ||
        fechaPago.trim().length === 0 ||
        monto === null ||
        monto === 0 ||
        (usuario === null && proveedor === null && cliente === null) ||
        tipoPago === null ||
      ...
      <div className="page-content">
        <Container fluid={true}>
          <Breadcrumbs
            ...
            breadcrumbItem={getValueBeadcrum().value}
            breadcrumbItemUrl={getValueBeadcrum().url}
          />
          <Row>
            <div className="col d-flex justify-content-end mb-3">
              {statesAccountingControl.map((estado, i) => (
                <div
                  key={i}
                  className={
                    estado === estadoRegistroContable
                      ? `states_cards active ${i == 0 ? 'first' : ''} ${i == statesAccountingControl.length - 1 ? 'last' : ''}`
                      : `states_cards ${i == 0 ? 'first' : ''} ${i == statesAccountingControl.length - 1 ? 'last' : ''}`
                  }
                >
                  <span>{estado}</span>
                </div>
            ...
            <div className="col mb-3 text-start">
              {estadoRegistroContable === 'BORRADOR' && (
                <>
                  <button
                    ...
                    onClick={() => {
                      onSaveRegistroContable('PENDIENTE');
                    }}
                  >
                    Confirmar
                  </button>
              ...
              {estadoRegistroContable === 'PENDIENTE' && (
                <button
                  type="button"
                  className="btn btn-info waves-effect waves-light me-2"
                  disabled={disablePagarOrden}
                  onClick={() => {
                    onSaveRegistroContable('PAGADO');
                  }}
                >
                  Pagar
                </button>
            ...
            {estadoRegistroContable === 'BORRADOR' && (
              <div className="col mb-3 text-end">
                <button
                  ...
                  disabled={disableGuardarOrden}
                  onClick={() => {
                    onSaveRegistroContable(estadoRegistroContable);
                  }}
                >
            ...
            <Row>
              ...
              <div className="col-md-3 col-sm-6 mb-3">
                ...
                <input
                  ...
                  type="number"
                  readOnly={estadoRegistroContable !== 'BORRADOR'}
                  id="monto"
                  pattern="[0-9]*"
                  inputMode="numeric"
                  value={monto}
              ...
              {estadoRegistroContable !== 'BORRADOR' && tipo === 'PAGAR' && (
                <div className="col-md-3 col-sm-6 mb-3">
                  ...
                  <input
                    className="form-control"
                    type="text"
                    disabled={estadoRegistroContable !== 'BORRADOR'}
                    id="Identificador"
                    value={
                      accountingControl.consecutivo ? accountingControl.consecutivo.consecutivo : ''
                    }
                  />
                </div>
              )}
                          ...
                          <table className="table">
                            <thead className="table-light">
                              <tr>
                                ...
                                {data_orden_compra.obtenerOrdenCompra.estadoPedido ===
                                  'Recibido' && <th>Cantidad recibida</th>}
                                <th>Descuento</th>
                                <th>Subtotal</th>
                                <th>Impuestos</th>
                                <th>Total</th>
                            ...
                            <tbody>
                              {data_orden_compra.obtenerOrdenCompra.lineasPedido.length > 0 &&
                                data_orden_compra.obtenerOrdenCompra.lineasPedido.map(
                                  (linea, index) => (
                                    <tr key={index}>
                                      ...
                                      {data_orden_compra.obtenerOrdenCompra.estadoPedido ===
                                        'Recibido' && <td>{linea.cantidadRecibida}</td>}
                                      <td>
                                        ₡{linea.descuento}{' '}
                                        {`${linea.porcentajeDescuento > 0 ? `(${linea.porcentajeDescuento}%)` : ''}`}
                                      </td>
                                      <td>₡{linea.subtotalSinImpuesto}</td>
                                      <td>
                                        ₡{linea.montoImpuestos}{' '}
                                        {`${linea.impuesto?.valor > 0 ? `(${linea.impuesto.valor}%)` : ''}`}
                                      </td>
