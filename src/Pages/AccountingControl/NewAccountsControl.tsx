...
const NewAccountsControl = (props: any) => {
  useEffect(() => {
  ...
  const contactsTypes: SelectOption<ContactType>[] = [
    { label: 'Usuario', value: 'usuario' as ContactType },
    { label: 'Cliente', value: 'cliente' as ContactType },
    { label: 'Proveedor', value: 'proveedor' as ContactType }
  ];

  const accontsTypes: SelectOption<AccountType>[] = [
    { label: 'Por pagar', value: 'PAGAR' as AccountType },
    { label: 'Por cobrar', value: 'COBRAR' as AccountType }
  ];
  ...
  const onSaveAccount = async () => {
    try {
      ...
      const input = {
        ...
        fechaRegistro:
          fechaRegistro !== null && fechaRegistro.trim().length > 0
      ...
      showInfoAlert({
        title: 'Oops',
        text: 'Ocurrió un error inesperado al guardar el registro contable',
        icon: 'error',
        timer: 3000,
        position: 'top-end'
      });
