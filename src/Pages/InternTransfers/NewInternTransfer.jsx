...
const NewInternTransfer = (props) => {
  ...
  const { loading: load_almacenes, data: data_almacenes } = useQuery(OBTENER_ALMACENES, {
    pollInterval: 1000
  });
  const { data: data_usuario } = useQuery(OBTENER_USUARIO_CODIGO, {
    variables: { codigo: localStorage.getItem('cedula') }
  });
  const [
    getWarehouseLines,
    { called: called_lineas_almacen, loading: load_lineas_almacen, data: data_lineas_almacen }
  ] = useLazyQuery(OBTENER_LINEAS_ALMACEN, { pollInterval: 1000 });
  ...
  useEffect(() => {
    ...
    if (data_lineas_almacen) {
      ...
      if (data_lineas_almacen.obtenerLineasAlmacen.length === 0) {
        showInfoAlert({
          title: 'Oops',
          text: 'No hay productos en el almacén que envía',
          icon: 'error',
          timer: 3000,
          position: 'center'
        });
        ...
        setLineasAlmacen(
          data_lineas_almacen.obtenerLineasAlmacen.map((l, i) => {
            return {
              ...l,
  ...
  const onSave = async () => {
    try {
      ...
      if (lineasTem && lineasTem.length > 0) {
        ...
        showInfoAlert(
          'Oops',
          'No ha colocado ninguna cantidad a transferir',
          'warning',
          3000,
          'top-end'
        );
      ...
      showInfoAlert(
        'Oops',
        'Ocurrió un error inesperado al registrar la transferencia interna',
        'error',
        3000,
        'top-end'
      );
        ...
        <div className="page-content">
          <Container fluid={true}>
            <Breadcrumbs
              ...
              breadcrumbItem="Transferencias internas"
              breadcrumbItemUrl="/internTransfers"
            />
            <Row>
        ...
        <Container fluid={true}>
          ...
          {lineasAlmacenCargadas && lineasAlmacen.length > 0 && (
            <Row>
              <div className="col mb-3 text-end">
                ...
                >
                  Atrás <i className="ri-arrow-left-line align-middle ms-2"></i>
                </button>
              </div>
              <div className="col mb-3 text-end">
                <button
          ...
          <Row>
            <div className="col-md-5 col-sm-12">
              ...
              <Row>
                <div className="col mb-3">
                  <label htmlFor="almacenDesde" className="form-label">
                    * Almacén que envía
                  </label>
                  {!lineasAlmacenCargadas ? (
                    ...
                    <input
                      ...
                      readOnly={true}
                      type="text"
                      value={almacenDesde.value.nombre}
                    />
                  )}
                </div>
              </Row>
              <Row>
                <div className="col mb-3">
                  <label htmlFor="almacenHasta" className="form-label">
                    * Almacén que recibe
                  </label>
                  {!lineasAlmacenCargadas ? (
                    <Select
                    ...
                    <input
                      ...
                      readOnly={true}
                      type="text"
                      value={almacenHasta.value.nombre}
                    />
                  )}
                </div>
              </Row>
              ...
              {!lineasAlmacenCargadas && almacenDesde !== null && almacenHasta !== null && (
                <Row>
                  <div className="col mb-3">
                    <button
                      disabled={disableCargar}
                      onClick={cargarLineasAlmacen}
                      type="button"
                      className="btn btn-outline-primary waves-effect waves-light w-100"
                    >
                      Cargar <i className="ri-save-line align-middle ms-2"></i>
                    </button>
                  </div>
                </Row>
              )}
                            ...
                            <table className="table table-hover table-striped mb-0">
                              ...
                              <tbody>
                                {lineasAlmacen.map((linea, i) => (
                                  <TrInternTransferLine
                                    key={`lineaAlmacen-${i}`}
                                    linea={linea}
                                    index={i}
                                    onHandleChange={onHandleCantidadTransferir}
                                  />
                                ))}
                              </tbody>
                            </table>
