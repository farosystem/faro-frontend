...
import {
  DELETE_PUESTO_LIMPIEZA,
  OBTENER_PUESTO_LIMPIEZAS
} from '../../services/PuestoLimpiezaService';
...
const CleaningJobs = ({ ...props }) => {
  ...
  const {
    loading: load_puesto_limpieza,
    error: error_puesto_limpieza,
    data: data_puesto_limpieza,
    refetch
  } = useQuery(OBTENER_PUESTO_LIMPIEZAS, { pollInterval: 1000 });
  ...
  const onDelete = (id, name) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        if (estado) {
          showInfoAlert({
            title: 'Puesto de limpieza eliminado',
            text: message,
            icon: 'success',
            timer: 3000,
            position: 'center'
