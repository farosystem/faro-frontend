...
const EditCleaningJob = ({ props, cleaningJob }) => {
  ...
  useEffect(() => {
    ...
    setUbicacion(
      cleaningJob.ubicacion
        ? { label: cleaningJob.ubicacion.nombre, value: cleaningJob.ubicacion.id }
        : null
    );
    setLineas(cleaningJob.areas);
  }, [cleaningJob]);

  const { loading: load_ubicaciones, data: data_ubicaciones } = useQuery(OBTENER_UBICACIONES, {
    pollInterval: 1000
  });
  ...
  const guardarLinea = () => {
    if (modoLinea === 'Agregar') {
      ...
      if (existe) {
        showInfoAlert({
          title: 'Oops',
          text: 'Ya existe una área con ese nombre',
          icon: 'warning',
          timer: 3000,
          position: 'center'
        });
  ...
  const onSave = async () => {
    try {
      ...
      const { data } = await actualizar({
        variables: { id: cleaningJob.id, input },
        errorPolicy: 'all'
      });
      ...
      showInfoAlert(
        'Oops',
        'Ocurrió un error inesperado al actualizar el puesto de limpieza',
        'error',
        3000,
        'top-end'
      );
        ...
        <div className="page-content">
          <Container fluid={true}>
            <Breadcrumbs
              ...
              breadcrumbItem="Puestos de limpieza"
              breadcrumbItemUrl="/cleaningjobs"
            />
            <Row>
            ...
            <div className="col-md-7 col-sm-12 mb-3">
              ...
              <Row className="align-items-end">
                <div className="col-md-8 col-sm-12 mb-3">
                  <label htmlFor="areaNombre">Nombre de área</label>
                  <input
                    id="areaNombre"
                    value={areaNombre}
