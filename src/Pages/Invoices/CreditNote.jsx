...
const InvoiceCreditNote = ({ ...props }) => {
  ...
  const { data: dataFacturasEmitidas } = useQuery(OBTENER_FACTURAS_EMITIDAS, {
    pollInterval: 1000
  });
    ...
    if (dataFacturasEmitidas) {
      if (dataFacturasEmitidas.obtenerFacturasEmitidas) {
        return dataFacturasEmitidas.obtenerFacturasEmitidas.filter((value, index) => {
          if (
            value.data &&
            value.data.DocElectronicos &&
            value.data.DocElectronicos.length > 0 &&
            value.data.DocElectronicos[0].Encabezado.TipoDocumento === '1'
          ) {
            ...
            if (clienteCedula.length > 0) {
              if (value.data && value.data.DocElectronicos.length > 0) {
                checkByNombreCedula =
                  value.data.DocElectronicos[0].Encabezado.Receptor.IdentificacionNumero.includes(
                    clienteCedula
                  ) ||
                  value.data.DocElectronicos[0].Encabezado.Receptor.Nombre.toLowerCase().includes(
                    clienteCedula.toLowerCase()
                  )
                    ? true
                    : false;
              }
            ...
            if (auxFechaFrom > 0) {
              if (value.response && value.response.Fecha && value.response.Fecha.includes('T')) {
                checkByFechaFrom =
                  new Date(value.response.Fecha.split('T')[0]) >= auxFechaFrom ? true : false;
              }
            }
            ...
            if (auxFechaTo > 0) {
              if (value.response && value.response.Fecha && value.response.Fecha.includes('T')) {
                checkByFechaTo =
                  new Date(value.response.Fecha.split('T')[0]) <= auxFechaTo ? true : false;
              }
            }
    ...
    <React.Fragment>
      <div className="page-content">
        ...
        <Modal isOpen={modalNota} toggle={toggleNota} size="xl">
          <ModalHeader toggle={toggleNota}></ModalHeader>
          <ModalBody>
