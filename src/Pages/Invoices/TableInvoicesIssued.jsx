...
const TableInvoicesIssued = ({ ...props }) => {
  ...
  const getEstadoHacienda = async (data, index) => {
    ...
    const response = await ffeClient.getDocument(body);
    ...
    if (response.result) {
      setDataMap((prevItems) => {
        ...
        const itemCopy = {
          ...
          response: {
            ...newItems[index].response,
            EstadoHacienda: response.response.DescripcionEstado
          }
        };
        newItems[index] = itemCopy;
        return newItems;
      });
      console.log(response.response.DescripcionEstado);
      if (response.response.DescripcionEstado === 'Aceptado') {
        return true;
      } else {
        return false;
      }
          ...
          {dataMap.map((invoice, i) => {
            return (
              <tr key={`invoiceIssued-${i}`}>
                ...
                <td>
                  {invoice.data.DocElectronicos.length > 0
                    ? dataDocumentTypes.obtenerFacturasParametrosByType.find(
                        (item) =>
                          item.id === invoice.data.DocElectronicos[0].Encabezado.TipoDocumento
                      ).value
                    : null}
                </td>
                <td>
                  {invoice.data.DocElectronicos.length > 0
                    ? invoice.data.DocElectronicos[0].Encabezado.Receptor.Nombre
                    : null}
                </td>
                <td>
                  {invoice.data.DocElectronicos.length > 0
                    ? invoice.data.DocElectronicos[0].Encabezado.Receptor.IdentificacionNumero
                    : null}
                </td>
                <td>
                  {!invoice.response.EstadoHacienda ? (
                    <button
                      ...
                      onClick={() => {
                        getEstadoHacienda(invoice.response, i);
                      }}
                    >
                        ...
                        <>
                          {invoice.response.EstadoHacienda === 'SIN ESTADO' ? (
                            <div className="d-flex" style={{ gap: '20px' }}>
                              <div>{invoice.response.EstadoHacienda}</div>
                              <button
                                type="button"
                                className="btn btn-rounded btn-info waves-effect waves-light me-3"
                                onClick={() => {
                ...
                {props.actions ? (
                  <td>
                    <ButtonIconTable
                      ...
                      color="warning"
                      onClick={async () => {
                        var value = await getEstadoHacienda(invoice.response, i);
                        console.log(value);
                        if (value) {
                          props.toggleNota(invoice);
                        } else {
                          props.toggleNota(false);
                        }
                      }}
                    />
                  </td>
