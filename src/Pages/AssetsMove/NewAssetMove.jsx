...
const NewAssetMove = () => {
  ...
  const { loading: load_activos, data: data_activos } = useQuery(OBTENER_ACTIVOS, {
    pollInterval: 1000
  });
  ...
  useEffect(() => {
    if (data_activos) {
      const data = data_activos.obtenerActivos.map((asset) => {
        return {
          label:
            asset.nombre + (asset.referenciaInterna ? ', ref: ' + asset.referenciaInterna : ''),
          value: asset.id
        };
      });
      setAssets(data);
  ...
  const onSaveAssetMove = async () => {
    try {
      ...
      showInfoAlert({
        title: 'Oops',
        text: 'Ocurrió un error inesperado al guardar el movimiento',
        icon: 'error',
        timer: 3000,
        position: 'center'
      });
        ...
        <div className="page-content">
          <Container fluid={true}>
            <Breadcrumbs
              ...
              breadcrumbItem={'Movimientos Activo'}
              breadcrumbItemUrl={`/assets`}
            />
            <Row>
      ...
      <div className="page-content">
        <Container fluid={true}>
          <Breadcrumbs
            title={'Nuevo Movimiento'}
            breadcrumbItem={'Movimientos Activo'}
            breadcrumbItemUrl={`${data_asset && id ? '/asset/movements/' + data_asset.obtenerActivo.nombre + '/' + id : '/assets/movements'}`}
          />
          <Row>
            <div className="col mb-3 text-end">
              <button
                disabled={disableSave}
                onClick={onSaveAssetMove}
                type="button"
          ...
          <Row>
            ...
            <div className="col-md-4 col-sm-12">
              <label className="col mb-2 form-label">* Activos</label>
              <Card className="p-2">
                <CardBody>
                  ...
                  <Row>
                    <ListInfo
                      ...
                      keys={['nombre', 'referenciaInterna']}
                      enableEdit={false}
                      enableDelete={true}
                      actionDelete={handleDeleteAsset}
                      mainKey={'id'}
                    />
                  </Row>
                </CardBody>
