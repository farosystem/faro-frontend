...
const AssetMove = () => {
  ...
  const {
    loading: loading_asset,
    error: error_asset,
    data: data_asset,
    startPolling,
    stopPolling
  } = useQuery(OBTENER_ACTIVO_CON_MOVIMIENTOS, { variables: { id: id }, pollInterval: 1000 });
      ...
      <React.Fragment>
        <div className="page-content">
          <Container fluid={true}>
            <Breadcrumbs
              title={'Movimientos Activo'}
              breadcrumbItem={'Activos'}
            ...
            <div className="col mb-3 text-end">
              <Link to={`/asset/newmovement/${id}`}>
                <button type="button" className="btn btn-primary waves-effect waves-light">
                  Agregar movimiento <i className="mdi mdi-plus align-middle ms-2"></i>
                </button>
              </Link>
            </div>
          </Row>
          ...
          <Row>
            <div className="col mb-3">
              <Card>
                <CardBody>
                  <DataList
                    data={data_asset.obtenerActivoConMovimientos.movimientos}
                    displayLength={9}
                    type="assetMove"
                  />
                </CardBody>
