...
const EditSupplyType = () => {
  ...
  const { id } = useParams();
  const {
    loading: loading_supply,
    error: error_supply,
    data: data_supply,
    startPolling,
    stopPolling
  } = useQuery(OBTENER_TIPO_PROVEDURIA_BY_ID, { variables: { id: id }, pollInterval: 1000 });
  ...
  const onClickSave = async () => {
    try {
      ...
      showInfoAlert(
        'Oops',
        'Ocurrió un error inesperado al guardar el tipo de proveeduría',
        'error',
        3000,
        'top-end'
      );
      ...
      <React.Fragment>
        <div className="page-content">
          <Container fluid={true}>
            <Breadcrumbs
              title="Editar Tipo de Proveeduría"
              breadcrumbItem="Gestion de Tipos de Proveeduría"
              breadcrumbItemUrl="/suppliertype"
            />
            <Row>
