...
import {
  convertirDataTipoProveeduriaExcel,
...
const SupplyType = ({ ...props }) => {
  ...
  const {
    ...
    data: tipos,
    refetch
  } = useQuery(OBTENER_TIPO_PROVEDURIA, { pollInterval: 1000 });
  ...
  const onDeletSupply = (id, tipo) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        if (estado) {
          showInfoAlert({
            title: 'Tipo de proveeduría eliminado',
            text: message,
            icon: 'success',
            timer: 3000,
            position: 'center'
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <Breadcrumb
            title="Gestion de Tipos de Proveeduría"
