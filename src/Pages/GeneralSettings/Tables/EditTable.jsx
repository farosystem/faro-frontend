...
const EditTable = (props) => {
  ...
  const {
    ...
    startPolling,
    stopPolling
  } = useQuery(OBTENER_MESA_BY_ID, { variables: { id: id }, pollInterval: 1000 });
  ...
  useEffect(() => {
    if (data_table && dataFloors) {
      ...
      const matchingTableType = tableTypes.find(
        (type) => type.value === data_table.obtenerMesaById.tipo
      );
      if (matchingTableType) {
        setTableType(matchingTableType);
      }
      const matchingFloor = dataFloors.obtenerPisos.find(
        (floor) => floor.id === data_table.obtenerMesaById.piso.id
      );
      if (matchingFloor) {
      ...
      <React.Fragment>
        <div className="page-content">
          <Container fluid={true}>
            <Breadcrumbs
              title="Editar Mesa"
              breadcrumbItem="Gestión de Mesas"
              breadcrumbItemUrl="/tables"
            />
            <Row>
