...
const EditRole = ({ props, role }) => {
  ...
  useEffect(() => {
    ...
    appModules.forEach((module) => {
      const foundModule = role.permisos.some(
        (permiso) => permiso.modulo.toLowerCase() === module.toLowerCase()
      );
      if (!foundModule) {
        tempPermissions = [
          ...tempPermissions,
          { modulo: module, editar: false, eliminar: false, agregar: false, ver: false }
        ];
      }
    });
  ...
  const onClickSave = async () => {
    try {
      ...
      showInfoAlert({
        title: 'Oops',
        text: 'Ocurrió un error inesperado al guardar el rol',
        icon: 'error',
        timer: 3000,
        position: 'center'
      });
