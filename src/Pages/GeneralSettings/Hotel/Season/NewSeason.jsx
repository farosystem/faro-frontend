...
const NewSeason = () => {
  ...
  useEffect(() => {
    setDisableSave(
      !(typeSeason && dateStart && dateEnd && validatePriceForPriceTypeRoom(priceTypeRoom))
    );
  ...
  const getFilteredTypeRoomByKey = (nombre) => {
    if (nombre !== '') {
      ...
      const filtered = typeRooms.filter((type) => type.nombre.toLowerCase().includes(value));
  ...
  const onClickSave = async () => {
    try {
      ...
      showInfoAlert(
        'Oops',
        'Ocurrió un error inesperado al guardar la temporada',
        'error',
        3000,
        'top-end'
      );
            ...
            <Row className="d-flex justify-content-between shadow_service rounded-5">
              ...
              <Col className="col-md-6 d-flex justify-content-center flex-wrap">
                <div className="col-md-12">
                  <CardBody>
                    <DataList
                      props={{ handlePriceChange, getFilteredTypeRoomByKey, priceTypeRoom }}
                      data={filterTypeRooms.length > 0 ? filterTypeRooms : typeRooms}
                      type="typeroomseason"
                      displayLength={5}
                    />
                  </CardBody>
