...
import { iconTypeService } from '@/constants/iconServiceTypes';
...
const EditTypeService = () => {
  ...
  const { id } = useParams();
  const {
    loading: loading_typeservice,
    error: error_typeservice,
    data: data_typeservice,
    startPolling,
    stopPolling
  } = useQuery(OBTENER_TIPOSSERVICIOSBYID, { variables: { id: id }, pollInterval: 1000 });
  ...
  const onClickSave = async () => {
    try {
      ...
      showInfoAlert({
        title: 'Oops',
        text: 'Ocurrió un error inesperado al actualizar el tipo de servicio',
        icon: 'error',
        timer: 3000
      });
      setDisableSave(false);
    ...
    if (iconTypeService) {
      iconTypeService.forEach((item) => {
        data.push({
          value: item.icon,
          label: (
            <label className={`${item.icon}`}>
              <span className="ms-2 fs-7">{item.label}</span>
            </label>
          ),
        ...
        <div className="page-content">
          <Container fluid={true}>
            <Breadcrumbs
              ...
              breadcrumbItem="Tipo de servicio"
              breadcrumbItemUrl="/hotelsettings/typeservice"
            />
            <Row>
              ...
              <Col className="col-md-8 d-flex flex-wrap">
                <div className="col-md-12 col-sm-9 m-2 d-flex">
                  ...
                  <div className="form-check ms-3 mt-4 d-flex align-items-center flex-row-reverse p-0 ps-4">
                    <label htmlFor="isSameAsCustomer" className="form-check-label ms-1">
                      Cuantificable
                    </label>
                    <input
                ...
                <div className="col-md-12 col-sm-9 m-2 d-flex justify-content-between">
                  ...
                  <div className="col-md-6 col-sm-12 ">
                    <label htmlFor="timeday" className="form-label">
                      * Hora del día{' '}
                    </label>
                    <Select
                      id="timeday"
