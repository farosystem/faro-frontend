...
const EditFloor = () => {
  ...
  const { id } = useParams();
  const {
    loading: loading_floor,
    error: error_floor,
    data: data_floor,
    startPolling,
    stopPolling
  } = useQuery(OBTENER_PISO_BY_ID, { variables: { id: id }, pollInterval: 1000 });
  ...
  const onClickSave = async () => {
    try {
      ...
      showInfoAlert({
        title: 'Oops',
        text: 'Ocurrió un error inesperado al guardar el Piso',
        icon: 'error',
        timer: 3000,
        position: 'center'
      });
      ...
      <React.Fragment>
        <div className="page-content">
          <Container fluid={true}>
            <Breadcrumbs
              title="Editar Piso"
              breadcrumbItem="Gestion de Pisos"
              breadcrumbItemUrl="/floors"
            />
            <Row>
