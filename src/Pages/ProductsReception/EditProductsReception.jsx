...
const EditProductsReception = ({ props, recepcion }) => {
  ...
  const { loading: load_lineas, data: data_lineas } = useQuery(OBTENER_LINEAS_RECEPCION_PEDIDO, {
    variables: { id: recepcion.id },
    pollInterval: 1000
  });
  ...
  const estadosRecepcion = ['Borrador', 'Confirmado'];
  ...
  const confirmarRecepcion = async () => {
    try {
      ...
      const { data } = await actualizarEstadoRecepcion({
        variables: { id: recepcion.id, estado: 'Confirmado' },
        errorPolicy: 'all'
      });
      ...
      showInfoAlert(
        'Oops',
        'Hubo un error inesperado al confirmar la recepción',
        'error',
        3000,
        'top-end'
      );
      ...
      <React.Fragment>
        <div className="page-content">
          <Container fluid={true}>
            <Breadcrumbs
              title="Editar recepción de producto"
              breadcrumbItem="Recepción de productos"
              breadcrumbItemUrl="/productsreception"
            />
            <Row>
                ...
                <div
                  ...
                  className={
                    estado === estadoRecepcion
                      ? `states_cards active ${i == 0 ? 'first' : ''} ${i == estadosRecepcion.length - 1 ? 'last' : ''}`
                      : `states_cards ${i == 0 ? 'first' : ''} ${i == estadosRecepcion.length - 1 ? 'last' : ''}`
                  }
                >
                  <span>{estado}</span>
                </div>
                  ...
                  <div className="col mt-2">
                    <div className="form-check">
                      <input
                        ...
                        value={almacenTodos}
                        onChange={() => {
                          setAlmacenTodos(!almacenTodos);
                        }}
                        id="warehouseCheck"
                      />
                      <label className="form-check-label" htmlFor="warehouseCheck">
