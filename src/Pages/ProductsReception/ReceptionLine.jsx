...
const ReceptionLine = ({ props, linea, estadoRecepcion, almacenes, almacen, almacenTodos }) => {
  ...
  const [almacenLinea, setAlmacenLinea] = useState(
    linea.almacen ? { value: linea.almacen, label: linea.almacen.nombre } : null
  );
    ...
    try {
      const { data } = await actualizar({
        variables: {
          ...
          almacen: alm ? alm.value.id : almacenLinea.value.id
        },
        errorPolicy: 'all'
      });
      ...
      showInfoAlert(
        'Oops',
        'Hubo un error inesperado al actualizar la cantidad',
        'error',
        3000,
        'top-end'
      );
