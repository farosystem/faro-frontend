...
export const convertDataStockExcel = (allData) => {
  ...
  for (let i = 0; i < allData.length; i++) {
    ...
    let row = [
      value.materia_prima.nombre,
      value.materia_prima.pais,
      calculateStockMovements(value.movimientos) + ' ' + value.materia_prima.unidad
    ];
...
export const convertDataStockReportExcel = (allData) => {
  let data = [['Nombre', 'País', 'Existencias']];
  for (let i = 0; i < allData.length; i++) {
    const value = allData[i];

    let row = [value.nombre, value.pais, value.existencias + ' ' + value.unidad];
...
export const convertDataComandaReportExcel = (allData) => {
  const data = [['ID', 'Fecha', 'Fecha finalizada', 'Mesa']];
  for (let i = 0; i < allData.length; i++) {
    const item = allData[i];
    data.push([
      item._id || '',
      item.fecha ? new Date(item.fecha).toLocaleDateString() : '',
      item.fechaFinalizada ? new Date(item.fechaFinalizada).toLocaleDateString() : '',
      item.mesa?.numero || ''
    ]);
  }
  return data;
...
export const convertDataMovimientosReportExcel = (allData) => {
  const data = [['ID', 'Fecha', 'Cliente', 'Total', 'Descuento', 'Mesa']];
  for (let i = 0; i < allData.length; i++) {
    const item = allData[i];
    data.push([
      item._id || '',
      item.fecha ? new Date(item.fecha).toLocaleDateString() : '',
      item.nombreFacturacion || '',
      item.total != null ? item.total : '',
      item.descuento != null ? item.descuento : '',
      (item.comanda && item.comanda.mesa && item.comanda.mesa.numero) || ''
    ]);
  }
  return data;
};
...
export const convertDataPurchaseOrdersExcel = (allData) => {
  ...
  for (let i = 0; i < allData.length; i++) {
    ...
    let row = [
      getFecha(value.fechaPedido),
      value.proveedor.empresa,
      value.estadoPedido,
      value.numeroComprobante | '',
      value.subtotal,
      value.impuestosMonto,
      value.total
    ];
...
export const convertDataProductReceptionExcel = (allData) => {
  ...
  for (let i = 0; i < allData.length; i++) {
    ...
    let row = [
      ...
      value.fechaEntrega ? getFecha(value.fechaEntrega) : ''
    ];
...
export const convertDataCleaningJobsExcel = (allData) => {
  ...
  for (let i = 0; i < allData.length; i++) {
    ...
    let row = [
      puestoLimpieza.nombre,
      puestoLimpieza.codigo,
      puestoLimpieza.ubicacion.nombre,
      areas
    ];
...
export const convertDataCleanlinessCheck = (allData) => {
  ...
  for (let i = 0; i < allData.length; i++) {
    ...
    console.log(areas);
    let row = [
      getFecha(check.fecha),
      check.puesto_limpieza.nombre,
      check.aprobado ? 'Sí' : 'No',
...
export const convertDataInternTransfers = (allData) => {
  ...
  for (let i = 0; i < allData.length; i++) {
    ...
    let row = [
      getFecha(internTransfer.fecha),
      internTransfer.almacenDesde?.nombre || '',
      internTransfer.almacenHasta?.nombre || '',
      internTransfer.usuario?.nombre || ''
    ];
...
export const convertirDataStockMoveExcel = (allData, nombre) => {
  let data = [
    ['Producto', nombre, '', 'Existencias', calculateStockMovements(allData)],
    [],
    ['Movimientos'],
    [],
    [
  ...
  for (let i = 0; i < allData.length; i++) {
    ...
    const fechaFabricacion = movimiento.fechaFabricacion
      ? getFecha(movimiento.fechaFabricacion)
      : '';
    const fechaVencimiento = movimiento.fechaVencimiento
      ? getFecha(movimiento.fechaVencimiento)
      : '';
    const cantidad = movimiento.cantidad;
    const existencias = movimiento.tipo === 'ENTRADA' ? movimiento.existencia : '';
    const precioUnidad =
      movimiento.tipo === 'ENTRADA'
        ? `${simbolosMoneda[movimiento.moneda]}${movimiento.precio_unidad}`
        : '';
    const precioTotal =
      movimiento.tipo === 'ENTRADA'
        ? `${simbolosMoneda[movimiento.moneda]}${movimiento.precio}`
        : '';
...
export const convertDataCashRegisterExcel = (allData) => {
  let data = [['Código', 'Nombre', 'Número', 'Estado', 'Módulo']];
  for (let i = 0; i < allData.length; i++) {
    const value = allData[i];

    let row = [value.codigo, value.nombre, value.numero, value.estado, value.modulo];
...
export const convertirDataRegistrosContablesExcel = (allData) => {
  ...
  for (let i = 0; i < allData.length; i++) {
    ...
    let row = [
      i,
      value.tipoRegistroContable,
      fechaRegistro,
      value.monto,
      value.tipoPago ? value.tipoPago : 'PENDIENTE'
    ];
...
export const convertirDataMovimientosActivosExcel = (allData) => {
  let data = [
    ['ID', 'Tipo', 'Identificador', 'Beneficiario', 'Fecha', 'Activo', 'Referencia interna']
  ];
  for (let i = 0; i < allData.length; i++) {
    const value = allData[i];
    let row = [
      ...
      value.beneficiario,
      getFechaTZ('fechaHora', value.fecha),
      value.activo.nombre,
      value.activo.referenciaInterna
    ];
